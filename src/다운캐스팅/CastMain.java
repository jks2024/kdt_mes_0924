package 다운캐스팅;
// 업캐스팅 : 자연스러운 방향이기 때문에 형변환 필요 없음 (부모 클래스의 참조 변수로 자식 객체를 바라 보는 것)
// 다운캐스팅 : 업캐스팅 되었건 객체를 원래 자식 타입으로 되돌리는 것
// instanceof : 객체가 특정 클래스의 인스턴스(객체) 확인하는 연산자 (결과가 true이면 다운캐스팅 가능)
...
public class CastMain {
    ...
    public void addAnimal() {
        list.add(new Animal());  // list가 Animal 타입으로 지정되어 있으므로, Animal클래스의 객체 또는 상속받은 객체는 올수 있음
